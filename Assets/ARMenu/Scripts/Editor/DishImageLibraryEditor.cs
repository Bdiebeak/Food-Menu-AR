...
namespace ARMenu.Scripts.Editor
{
    ...
    class DishImageLibraryEditor : UnityEditor.Editor
    {
        ...
        SerializedProperty m_ReferenceImageToDishNodes;

        void OnEnable()
        {
            m_ReferenceImages = serializedObject.FindProperty("m_Images");
            m_ReferenceImageToDishNodes = serializedObject.FindProperty("imageToDishNodes");
        }
        // ---
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (indexToRemove > -1)
            {
                ...
                m_ReferenceImageToDishNodes.DeleteArrayElementAtIndex(indexToRemove);
            }
            // ---
            ...
            if (Content.addButton)
            {
                ...
                // --- MODIFIED
                (target as DishImageLibrary).imageToDishNodes.Add(new ReferenceImageToDishNode());
                // ---
        ...
        bool ReferenceImageField(int index)
        {
            ...
            // --- MODIFIED
            var dishLibrary = target as DishImageLibrary;
            Dish referenceDish = dishLibrary.imageToDishNodes[index].dish;
            bool wasDishUpdated = false;
            // ---
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                // --- MODIFIED
                using (var dishCheck = new EditorGUI.ChangeCheckScope())
                {
                    referenceDish = DishField(referenceDish);
                    wasDishUpdated = dishCheck.changed;
                }
                // ---
            ...
            using (var changeCheck = new EditorGUI.ChangeCheckScope())
            {
                ...
                // --- MODIFIED
                if (changeCheck.changed || wasTextureUpdated || wasDishUpdated)
                {
                    ...
                    Undo.RecordObject(target, "Update references");
                    library.SetTexture(index, texture, keepTexture);
                    dishLibrary.imageToDishNodes[index].arImage = texture;
                    dishLibrary.imageToDishNodes[index].dish = referenceDish;
                    EditorUtility.SetDirty(target);
                }
                // ---
            }
